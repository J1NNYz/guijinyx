...
do
    ...
    function Funcs:AddText(Text, DoesWrap)
        local Textz = {};
    
        local Groupbox = self;
        local Container = Groupbox.Container;
    
        local TextzLabel = Library:CreateLabel({
            Size = UDim2.new(1, -4, 0, 15);
            TextSize = 14;
            Text = Center and ([[\\ ]] .. Text .. " //") or Text;
            RichText = true,
            TextXAlignment = Center and Enum.TextXAlignment.Center or Enum.TextXAlignment.Center;
            ZIndex = 5;
            Parent = Container;
        });
    
        Textz.TextLabel = TextzLabel;
        Textz.Container = Container;
    
        if (not DoesWrap) then
            setmetatable(Textz, BaseAddons);
        end

        function Textz:ChangeText(Text)
            TextzLabel.Text = ([[\\ ]] .. Text .. " //")

            if DoesWrap then
                local Y = select(2, Library:GetTextBounds(Text, Enum.Font.Code, 14, Vector2.new(TextzLabel.AbsoluteSize.X, math.huge)))
                TextzLabel.Size = UDim2.new(1, -4, 0, Y)
            end
...
function Library:SetWatermark(Text)
    local X, Y = Library:GetTextBounds(Text, "Code", 14);
...
function Library:Notify(Text, Time)
    local XSize, YSize = Library:GetTextBounds(Text, "Code", 14);
